            ...
            if save then
                ...
                if folder ~= nil then
                    ...
                    elseif type( folder ) == 'table' then
                        app:logWarning( "*** Run save panel should return string." ) -- ###1
                        folders = folder
                    else
                ...
                if folders ~= nil and type( folders ) == 'string' then -- this shouldn't be happening, but I think there must be a new twist in Lr4, or something. ###1
                    folders = { folders }
                end
            end
...
function Dialog:_selectFile( param, props, name, _out )
    ...
    app:call( Call:new{ name="Select file", main=function( call )
        ...
        if win_env then
            ...
            args.prompt = nil -- ###1 param.prompt or "OK" - dunno if this helps on Mac(/Lion) yet, but its not working as is, so it can't hurt...
        end
        ...
        if _out then
            ...
            if files ~= nil then
                if type( files ) == 'string' then -- this shouldn't be happening, but I think there must be a new twist in Lr4, at least on Mac. ###1 dunno if this might happen, but the folder case inspired this clause.
                    files = { files }
                elseif type( files ) == 'table' then
